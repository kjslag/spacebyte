...
# requirements: wandb, torch, tiktoken, datasets, zstandard
...
class TrainConfig:
    ...
    # data
    data_seed: int = 0
    dataset: str = 'openwebtext' # 'pile-uncopyrighted' 'pg19'
    batch_size: str = '1.2*128*256/d' # optimized for A100x1 # 'test'
    ...
    final_eval_iters: str = 'min(iters/30 * B/mB, 2**20 / mB)' # number of micro-batches for the final evaluation
...
class Train:
    ...
    def __init__(self, model_or_config, train_config: TrainConfig, checkpoint=None, verbose=None):
        ...
        if isinstance(c.batch_size, str):
            d = mc.d_model
            c.batch_size = max(1, 2**round(math.log2(eval(c.batch_size))))
        B = c.batch_size
        ...
        if isinstance(c.final_eval_iters, str):
            c.final_eval_iters = round(eval(c.final_eval_iters))
